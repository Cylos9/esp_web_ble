...
/* GATT define
*/
#define GATTS_TABLE_TAG "GATTS_LIGHT_CONTROL"
...
#define GATTS_BRIG_CHAR_VAL_LEN_MAX  3
#define GATTS_CFG_CHAR_VAL_LEN_MAX   1
...
uint16_t light_control_handle_table[LC_IDX_NB];
...
static struct gatts_profile_inst light_control_profile_tab[PROFILE_NUM] = {
    [PROFILE_APP_IDX] = {
...
/* Service */
static const uint16_t GATTS_LC_SERVICE_UUID      = 0x00FF;
static const uint16_t GATTS_BRIG_CHAR_UUID       = 0xFF01;
static const uint16_t GATTS_CFG_CHAR_UUID       = 0xFF02;
...
static const uint8_t led_brig_init_val[3]         = {0x31, 0x30, 0x30};


/* Full Database Description - Used to add attributes into the database */
static const esp_gatts_attr_db_t gatt_db[LC_IDX_NB] =
{
    // Service Declaration
    [LC_IDX_SVC]        =
    {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&primary_service_uuid, ESP_GATT_PERM_READ,
      sizeof(uint16_t), sizeof(GATTS_LC_SERVICE_UUID), (uint8_t *)&GATTS_LC_SERVICE_UUID}},
    ...
    {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_BRIG_CHAR_UUID, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_BRIG_CHAR_VAL_LEN_MAX, sizeof(led_brig_init_val), (uint8_t *)led_brig_init_val}},
    ...
    {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&character_client_config_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      sizeof(uint16_t), sizeof(brightness_char_ccc), (uint8_t *)brightness_char_ccc}},
    ...
    /* Characteristic Value */
    [LC_IDX_CFG_CHAR_VAL]  =
    {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CFG_CHAR_UUID, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_CFG_CHAR_VAL_LEN_MAX, sizeof(led_cfg_init_val), (uint8_t *)led_cfg_init_val}},
};
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        case ESP_GATTS_REG_EVT:{
            ...
            adv_config_done |= SCAN_RSP_CONFIG_FLAG;
    #endif
            esp_err_t create_attr_ret = esp_ble_gatts_create_attr_tab(gatt_db, gatts_if, LC_IDX_NB, SVC_INST_ID);
            if (create_attr_ret){
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            ...
            else if (param->add_attr_tab.num_handle != LC_IDX_NB){
                ESP_LOGE(GATTS_TABLE_TAG, "create attribute table abnormally, num_handle (%d) \
                        doesn't equal to LC_IDX_NB(%d)", param->add_attr_tab.num_handle, LC_IDX_NB);
            }
            else {
                ESP_LOGI(GATTS_TABLE_TAG, "create attribute table successfully, the number handle = %d\n",param->add_attr_tab.num_handle);
                memcpy(light_control_handle_table, param->add_attr_tab.handles, sizeof(light_control_handle_table));
                esp_ble_gatts_start_service(light_control_handle_table[LC_IDX_SVC]);
            }
            break;
        }
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            light_control_profile_tab[PROFILE_APP_IDX].gatts_if = gatts_if;
        } else {
    ...
    do {
        ...
        for (idx = 0; idx < PROFILE_NUM; idx++) {
            ...
            if (gatts_if == ESP_GATT_IF_NONE || gatts_if == light_control_profile_tab[idx].gatts_if) {
                if (light_control_profile_tab[idx].gatts_cb) {
                    light_control_profile_tab[idx].gatts_cb(event, gatts_if, param);
                }
            }
        }
    } while (0);
